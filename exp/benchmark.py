...
class StreamingChunkCollector:
    ...
    def on_request_output(self, request_output: RequestOutput) -> None:
        ...
        for output in request_output.outputs:
            ...
            # Only a non-whitespace delta can create a new sentence boundary.
            # Otherwise whitespace streamed after a sentence-ending token would
            # be flushed as its own zero-word chunk because the cumulative text
            # still ends with the previous punctuation mark.
            if text_delta.strip() and self._is_sentence_boundary(text):
...
async def run_vllm_async(
    ...
    async with build_async_engine_client_from_engine_args(engine_args) as llm:
        ...
        for request in requests:
            ...
            max_tokens = min(generation_max_tokens, model_config.max_model_len - prompt_len)
            if max_tokens <= 0:
                raise ValueError(
            ...
            configured_max_tokens = sampling_kwargs.get("max_tokens")
            if configured_max_tokens is None:
                sampling_kwargs["max_tokens"] = max_tokens
            else:
                sampling_kwargs["max_tokens"] = min(int(configured_max_tokens), max_tokens)
        ...
        # Launch all requests and collect sentence chunks from streamed outputs.
...
def add_cli_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        default=1024,
        help=(
            "Safety cap for generation length when the model generation_config "
            "does not define max_new_tokens. Sampling values come from the "
...
def main(args: argparse.Namespace) -> None:
    ...
    write_jsonl(slack_output_jsonl, records)
    ...
    if args.chunk_output_jsonl:
        write_jsonl(args.chunk_output_jsonl, chunk_rows)
    if args.chunk_output_csv:
        write_csv(args.chunk_output_csv, chunk_rows, columns=SLACK_COLUMNS)
    ...
    if args.chunk_output_jsonl or args.chunk_output_csv:
        print(
            f"Human slack chunks: {len(chunk_rows)} rows -> "
            f"{args.chunk_output_jsonl or args.chunk_output_csv}"
        )
    ...
    if args.output_json:
        results = {
            ...
            "chunk_output_jsonl": args.chunk_output_jsonl,
            "chunk_output_csv": args.chunk_output_csv,
            "apply_chat_template": args.apply_chat_template,
            "generation_max_tokens": args.generation_max_tokens,
            **timeline_summary,
        }
