...
METRICS = (
    ("ttft_p50", "h2_ttft_p50_{mode}"),
    ("queue_stall_p50", "queue_stall_p50_{mode}"),
    ("tpot_p50", "h4_tpot_p50_{mode}"),
    ("neg_slack_ratio", "h3_slack_neg_ratio_{mode}"),
)
...
def main() -> None:
    ...
    rows = [
        ("TTFT p50 (s)",       "ttft_p50",         1.0,  "{:.3f}"),
        ("queue stall p50 (s)","queue_stall_p50",  1.0,  "{:.3f}"),
        ("TPOT p50 (ms)",      "tpot_p50",         1000, "{:.2f}"),
        ("neg slack ratio (%)","neg_slack_ratio",  100,  "{:.4f}"),
    ]
    ...
    print(f"{'mean neg / total':<22s}" + "".join(
        f"{(f'{statistics.mean([n for n,t in per_mode_neg[m]]):.1f} / {statistics.mean([t for n,t in per_mode_neg[m]]):.0f}' if per_mode_neg[m] else 'n/a'):>26s}"
        for m in MODES
    ))
