#!/usr/bin/env bash
# Add run_2 and run_3 to output_sweep_noabatch.
# For each (model, consume, cap, mode), runs all 4 rates [8, 16, 24, 32] in one engine load.
# 30 cells × 2 modes × 2 runs = 120 engine loads total.
# Splits into 4 phases for LPT balance.
set -euo pipefail
cd /workspace/mlsys
...
# Cost: cap × model_factor (9B:1x, 35B:3x); SSLO ≈ baseline cost.
# Cells: "model_slug|model_hf|consume_mode|tts_slug|tts_model|cap|mode|cost"
build_cells_for_model_run() {
...
while IFS= read -r line; do NINE_B_CELLS+=("$line"); done < <(
  build_cells_for_model_run "Qwen3.5-9B" "Qwen/Qwen3.5-9B" 1)

THIRTY5_B_CELLS=()
while IFS= read -r line; do THIRTY5_B_CELLS+=("$line"); done < <(
  build_cells_for_model_run "Qwen3.5-35B-A3B" "Qwen/Qwen3.5-35B-A3B" 3)

# Phase 1: 9B run_2
# Phase 2: 35B run_2
# Phase 3: 9B run_3
# Phase 4: 35B run_3
run_phase "Phase 1 (9B run_2)"   2 "${NINE_B_CELLS[@]}"
run_phase "Phase 2 (35B run_2)"  2 "${THIRTY5_B_CELLS[@]}"
run_phase "Phase 3 (9B run_3)"   3 "${NINE_B_CELLS[@]}"
run_phase "Phase 4 (35B run_3)"  3 "${THIRTY5_B_CELLS[@]}"

echo "===== ALL RUN_2 + RUN_3 DONE ====="
