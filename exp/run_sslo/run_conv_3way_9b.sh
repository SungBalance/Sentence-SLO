#!/usr/bin/env bash
# 3-way sweep for best/worst case on 9B with conversation_only.
#   modes: baseline, sslo_mlp (abatch=T), sslo_mlp_noabatch (abatch=F)
#   cases: best=cap32 r256, worst=cap256 r128 (9B is fast → stress at high cap)
...
ROOT="${OUTPUT_ROOT:-exp/run_sslo/output/conv_3way_9b}"
rm -rf "$ROOT"
mkdir -p "$ROOT"
