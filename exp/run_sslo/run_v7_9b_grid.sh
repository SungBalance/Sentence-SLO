#!/usr/bin/env bash
# 9B grid: cap{64,256} × rate{2,16} × modes{baseline, sslo_mlp, sslo_mlp_noabatch}
# 12 jobs, 4 GPU round-robin.
set -euo pipefail
cd /workspace/mlsys
# HF_TOKEN must be set in the environment by the caller (gated lmsys dataset).

ROOT=exp/run_sslo/output/v7_9b_grid
...
# Build job list: cap × rate × mode
jobs=()
for cap in 64 256; do
  for rate in 2 16; do
