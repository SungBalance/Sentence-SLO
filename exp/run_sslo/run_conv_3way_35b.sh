#!/usr/bin/env bash
# 3-way × 1 run for 35B (Qwen3.5-35B-A3B MoE).
#   modes: baseline, sslo_mlp (abatch=T), sslo_mlp_noabatch (abatch=F)
#   cases: best=cap32 r=128, worst=cap128 r=64
#   runs : 1 (seed=43)
# Total 6 jobs distributed across 4 GPUs round-robin.
set -euo pipefail
...
ROOT="${OUTPUT_ROOT:-exp/run_sslo/output/conv_3way_35b}"
...
launch() {
  ...
  env $abatch_env \
    ...
    bash exp/run_sslo/run_test.sh "$kind" "$cap" "$MODEL" \
    > "$outdir/run.log" 2>&1
}
