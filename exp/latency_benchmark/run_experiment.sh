...
OUTPUT_ROOT="${EXP_ROOT}/latency_benchmark/outputs"
...
for MAX_NUM_SEQS in "${MAX_NUM_SEQS_VALUES[@]}"; do
    ...
    (
      for CHUNK_UNIT in "${CHUNK_UNITS[@]}"; do
        ...
        run_on_gpu "${BENCH_CONTAINER}" "${GPU}" \
          python3 "${EXP_ROOT}/latency_benchmark/benchmark.py" \
          --model                  "${MODEL}" \
  ...
  for PID in "${PIDS[@]}"; do
    wait "${PID}" || {
      echo "Benchmark failed (max_num_seqs=${MAX_NUM_SEQS}, PID=${PID})" >&2
      exit 1
    }
  done
done
